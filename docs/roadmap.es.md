...
Entregado: semantica portable de delayed release, prioridades locales a cada queue, deduplicacion explicita de admision local a cada queue, tags diagnosticos acotados, identidad versionada por dispatch con checks de integridad canonica del payload, contexto final de admision verificado por registry, graceful draining BullMQ reforzado, dispatch after-commit best-effort explicito, middleware opt-in `WithoutOverlapping`, `RateLimited` y `ThrottlesExceptions` de ventana fija, autorizacion por ability configurada por el worker confiable, un contrato portable de delegacion autenticada y chains secuenciales acotadas en los adapters memory, testing y BullMQ. Las chains preparan eager cada step mediante admision final, solo avanzan tras exito, ocultan credenciales futuras en la inspeccion de payload de dead letters y conservan handoff at-least-once sin declarar atomicidad entre queues. La delegacion vincula credenciales opacas de aplicacion con allowlist de issuer, audience, alias exacto, job canonico, queue real, dispatch y payload antes de recargar delegacion/principal/membership actuales en cada ejecucion y failed hook. Crypto, keys/KMS, formatos de credencial y stores durables de identidad/revocacion siguen siendo adapters de aplicacion; la redelivery exacta sigue siendo valida bajo ejecucion at-least-once y los efectos requieren idempotencia. La candidata restante son batches durables con progreso y hooks de cancelacion acotados. Esto no debe convertir workflows en un motor DAG implicito.
