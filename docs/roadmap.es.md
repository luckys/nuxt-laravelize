...
Entregado: semantica portable de delayed release, prioridades locales a cada queue, deduplicacion explicita de admision local a cada queue y middleware opt-in `WithoutOverlapping` y `RateLimited` en los adapters memory y BullMQ, con metadata acotada, politica fail-closed para cache distribuido y observabilidad consciente de releases. Quedan como candidatos chains secuenciales, batches durables con progreso/cancelacion, dispatch after-commit, tags, draining, `ThrottlesExceptions` y requisitos de tenant/principal. La metadata durable debe ser versionada, acotada y compatible con ejecucion at-least-once. Esto no debe convertir workflows en un motor DAG implicito.
