...
Entregado: semantica portable de delayed release, prioridades locales a cada queue, deduplicacion explicita de admision local a cada queue, tags diagnosticos acotados, graceful draining BullMQ reforzado, dispatch after-commit best-effort explicito, middleware opt-in `WithoutOverlapping`, `RateLimited` y `ThrottlesExceptions` de ventana fija, y autorizacion por ability configurada por el worker confiable y reevaluada en cada ejecucion y failed hook. El bridge rechaza tratar actor o tenant serializados como credenciales y exige autenticacion de principal de queue y recarga de grants especificas de la aplicacion. Quedan como candidatos chains secuenciales, batches durables con progreso/cancelacion y un contrato portable de delegacion autenticada/pertenencia tenant. La metadata durable debe ser versionada, acotada y compatible con ejecucion at-least-once. Esto no debe convertir workflows en un motor DAG implicito.
