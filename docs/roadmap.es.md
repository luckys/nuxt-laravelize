...
Entregado: delayed release portable, prioridades y deduplicacion locales a la queue, tags acotados, identidad de dispatch versionada y admision final, draining BullMQ reforzado, dispatch after-commit best-effort, middleware, autorizacion configurada por worker, delegacion autenticada, chains secuenciales acotadas y batches acotados en una sola queue para adapters memory, testing y BullMQ. Los batches admiten eager entre 1 y 100 children independientes, exponen contadores fijos y cancelacion cooperativa, y derivan status durable de dependencies retained del flow BullMQ sin prometer interrupcion, rollback, exactly-once ni historial permanente. Memory sigue siendo volatil y el fake solo sirve para testing. Branches, nesting, fan-out entre queues y DAGs dinamicos quedan fuera y pertenecen a workflows.
