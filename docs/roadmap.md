...
Delivered: portable delayed-release semantics, queue-local priorities, explicit queue-local admission deduplication, and opt-in `WithoutOverlapping` and `RateLimited` middleware across the in-memory and BullMQ adapters, with bounded metadata, distributed-cache fail-closed policy, and release-aware observability. Remaining candidates include sequential chains, durable batches with progress/cancellation hooks, after-commit dispatch, tags, graceful draining, `ThrottlesExceptions`, and tenant/principal requirements. Durable metadata must be versioned, bounded, and compatible with at-least-once execution. This capability should not turn workflows into an implicit DAG engine.
