...
Delivered: portable delayed-release semantics, queue-local priorities, explicit queue-local admission deduplication, bounded diagnostic tags, versioned per-dispatch identity with canonical payload integrity checks, a registry-verified final admission context, hardened graceful BullMQ draining, explicit best-effort after-commit dispatch, opt-in `WithoutOverlapping`, `RateLimited`, and fixed-window `ThrottlesExceptions` middleware, trusted worker-configured ability authorization, a portable authenticated delegation contract, and bounded sequential chains across the memory, testing and BullMQ adapters. Chains eagerly prepare every step through final admission, advance only after success, redact future credentials from dead-letter payload inspection and retain at-least-once handoff semantics without claiming cross-queue atomicity. Delegation binds opaque application credentials to an issuer allowlist, audience, exact alias, canonical job, actual queue, dispatch and payload before reloading current delegation/principal/membership state on every process and failed-hook execution. Crypto, keys/KMS, credential formats and durable identity/revocation stores remain application adapters; exact redelivery remains valid under at-least-once execution and effects require idempotency. The remaining candidate is durable batches with bounded progress and cancellation hooks. This capability should not turn workflows into an implicit DAG engine.
