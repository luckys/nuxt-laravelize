...
Delivered: portable delayed-release semantics, queue-local priorities and admission deduplication, bounded diagnostic tags, versioned dispatch identity and final admission, hardened BullMQ draining, best-effort after-commit dispatch, queue middleware, worker-configured authorization, authenticated delegation, bounded sequential chains, and bounded same-queue batches across memory, testing and BullMQ adapters. Batches eagerly final-admit 1-100 independent children, expose fixed progress counters and cooperative cancellation, and use retained BullMQ flow dependencies for durable status without claiming interruption, rollback, exactly-once effects or permanent history. Memory remains volatile and the fake remains testing-only. Branches, nesting, cross-queue fan-out and dynamic DAG behavior remain explicit non-goals and belong in workflows.
