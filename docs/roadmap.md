...
Delivered: portable delayed-release semantics, queue-local priorities, explicit queue-local admission deduplication, bounded diagnostic tags, versioned per-dispatch identity with canonical payload integrity checks, a registry-verified final admission context for application credential issuers, hardened graceful BullMQ draining, explicit best-effort after-commit dispatch, opt-in `WithoutOverlapping`, `RateLimited`, and fixed-window `ThrottlesExceptions` middleware, plus trusted worker-configured ability authorization re-evaluated on every process and failed-hook execution. The authorization bridge refuses to treat serialized actor, tenant, dispatch ID or unkeyed fingerprint as credentials and requires application-specific queue principal authentication and current grant reload. Remaining candidates include sequential chains, durable batches with progress/cancellation hooks, and a portable authenticated delegation/tenant-membership contract that binds a verifiable credential to dispatch, canonical job, trusted queue/audience and payload. Durable metadata must be versioned, bounded, and compatible with at-least-once execution. This capability should not turn workflows into an implicit DAG engine.
