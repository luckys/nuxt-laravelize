...
Delivered: portable delayed-release semantics, queue-local priorities, explicit queue-local admission deduplication, bounded diagnostic tags, hardened graceful BullMQ draining, explicit best-effort after-commit dispatch, opt-in `WithoutOverlapping`, `RateLimited`, and fixed-window `ThrottlesExceptions` middleware, plus trusted worker-configured ability authorization re-evaluated on every process and failed-hook execution. The authorization bridge refuses to treat serialized actor or tenant provenance as credentials and requires application-specific queue principal authentication and current grant reload. Remaining candidates include sequential chains, durable batches with progress/cancellation hooks, and a portable authenticated delegation/tenant-membership contract. Durable metadata must be versioned, bounded, and compatible with at-least-once execution. This capability should not turn workflows into an implicit DAG engine.
