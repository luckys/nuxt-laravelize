...
Delivered: portable delayed-release semantics, queue-local priorities, explicit queue-local admission deduplication, bounded diagnostic tags, hardened graceful BullMQ draining, explicit best-effort after-commit dispatch, and opt-in `WithoutOverlapping`, `RateLimited`, and fixed-window `ThrottlesExceptions` middleware across the in-memory and BullMQ adapters, with bounded metadata, distributed-cache fail-closed policy, and release-aware observability. Remaining candidates include sequential chains, durable batches with progress/cancellation hooks, and tenant/principal requirements. Durable metadata must be versioned, bounded, and compatible with at-least-once execution. This capability should not turn workflows into an implicit DAG engine.
