...
Ejecuta periodicamente `new WorkflowWakeReconciler(durableWorkflowStore, durableReliabilityStore).reconcileStore({ pageSize: 100 })` para reparar wake-ups dead o perdidos operacionalmente. El scan acotado recarga estado autoritativo y agrega un ID de wake nuevo despues de cualquier lease activo o deadline de retry de negocio; nunca revive ni modifica la fila dead anterior. Usa `reconcile(ids)` con un indice propio de la aplicacion. Los fallos por workflow se devuelven sin abortar las paginas posteriores.
