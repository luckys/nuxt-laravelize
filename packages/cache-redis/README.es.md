...
Adapter opcional Node-only para Redis/Valkey (no incluido en el preset)
...
### Usa Redis o Valkey como cache compartido

Crea un `RedisCache` con un prefijo obligatorio de aplicacion y registralo mediante `cacheToken`. El paquete no inicia ni cierra la conexion, para que la aplicacion controle el ciclo de vida del cliente.
