...
async function expectMigration<D extends MigrationDialect>(dialect: D, source: MigrationSource<D>, name: string) {
  ...
  expect(execute).toHaveBeenCalledWith(expect.stringContaining('idempotency_records'))
}

describe('idempotency migration sources', () => {
  it('selects and loads the PostgreSQL migration', () => expectMigration('postgresql', postgresMigrationSource, '0000_idempotency_postgres'))
  it('selects and loads the SQLite migration', () => expectMigration('sqlite', sqliteMigrationSource, '0001_idempotency_sqlite'))
})
