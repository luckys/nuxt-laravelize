...
Call `process(id)` from a worker to perform at most one attempt, or `run(id)` for bounded local draining. Retry times use the injected `Clock` and `RetrySchedule`, enabling deterministic workers and tests. Cancellation is cooperative and observed between attempts; it does not interrupt a handler. If cancellation races a successful in-flight handler, its JSON-safe output is committed with the cancellation flag and then compensated. Committed compensatable steps are compensated in reverse order; otherwise the workflow becomes `cancelled`. Compensation retries and resumes like forward execution. Inputs and successful step outputs are runtime-validated JSON-safe values.
