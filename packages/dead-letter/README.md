...
These APIs are global-operator APIs unless an application independently enforces tenant scope. Applications must enforce `dead-letters.list`, `dead-letters.view`, `dead-letters.view-payload`, `dead-letters.retry`, `dead-letters.discard`, and the stronger `dead-letters.retry-inbox` ability. Never derive authorization from envelope actor or tenant hints. Payload, tenant hints, and error summaries are separately opt-in and may contain sensitive domain data even after best-effort secret redaction; grant error-summary access separately.
