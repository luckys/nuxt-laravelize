...
Contratos portables para gestionar mensajes fallidos por origen

## Instalacion

```bash
pnpm add @nuxt-laravelize/dead-letter
```

## Uso especifico del package


### Gestiona mensajes fallidos mediante un adapter explicito

Registra exactamente un adapter dead-letter y autoriza cada operacion en el boundary de la aplicacion. Payloads y resumenes de error son capacidades sensibles separadas; el actor o tenant del envelope nunca es una credencial de autorizacion.

```ts
import { DeadLetterManager } from '@nuxt-laravelize/dead-letter'
...
[`@nuxt-laravelize/reliability`](../reliability/README.es.md), [`@nuxt-laravelize/dead-letter-operations`](../dead-letter-operations/README.es.md), [`@nuxt-laravelize/queue-bullmq`](../queue-bullmq/README.es.md).
