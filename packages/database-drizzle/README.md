...
Drizzle transaction manager adapter for Nuxt Laravelize
...
### Run an explicit Drizzle transaction

The adapter keeps the session visible to application repositories and runs `afterCommit` hooks only after the native transaction confirms. Use the synchronous variant only with a synchronous SQLite driver.
...
[`@luckys_luis/nuxt-laravelize-database`](../database/README.md), [`@luckys_luis/nuxt-laravelize-reliability-drizzle`](../reliability-drizzle/README.md).
