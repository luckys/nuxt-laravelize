...
```bash
pnpm add @luckys_luis/nuxt-laravelize-dead-letter-operations
```
...
### Enable the fail-closed operations console

The dashboard is opt-in, uses exact canonical origins and literal non-overlapping paths, and registers no adapter or permissive ability for you. Configure the central authorization registry and the adapter registry in application code.
...
[`@luckys_luis/nuxt-laravelize-dead-letter`](../dead-letter/README.md), [`@luckys_luis/nuxt-laravelize-authorization`](../authorization/README.md).
