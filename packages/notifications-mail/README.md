...
### Add the validated mail channel

The notification defines content through `toMail()`, while the destination always comes from `routeNotificationFor("mail")`. The channel rejects header injection and forwards locale, abort, and idempotency metadata to the configured mailer.
...
[`@luckys_luis/nuxt-laravelize-notifications`](../notifications/README.md), [`@luckys_luis/nuxt-laravelize-mail`](../mail/README.md).
