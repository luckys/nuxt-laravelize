...
Durable workflow scheduling over Laravelize Queue
...
### Schedule workflow transitions through a queue

Queue payloads contain only the workflow ID. The worker reloads the authoritative row and claims by revision and lease; business retry deadlines become delayed successors while transport failures use queue retries.
...
[`@luckys_luis/nuxt-laravelize-workflows`](../workflows/README.md), [`@luckys_luis/nuxt-laravelize-queue`](../queue/README.md).
