...
Tenant-fenced realtime broadcast channel for Nuxt Laravelize notifications

## Install

```bash
pnpm add @luckys_luis/nuxt-laravelize-notifications-broadcast
```

```ts
// nuxt.config.ts
export default defineNuxtConfig({
  modules: ['@luckys_luis/nuxt-laravelize-notifications-broadcast'],
})
```
...
### Deliver a tenant-fenced broadcast notification

The notification chooses only its type, version, and bounded data. The recipient route supplies the opaque recipient and tenant identity; the package derives the private channel and fixed event name.
...
[`@luckys_luis/nuxt-laravelize-notifications`](../notifications/README.md), [`@luckys_luis/nuxt-laravelize-broadcasting-pusher`](../broadcasting-pusher/README.md).
