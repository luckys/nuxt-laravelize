...
Adapter filesystem opcional y nativo del binding R2, sin AWS SDK

## Instalacion

```bash
pnpm add @luckys_luis/nuxt-laravelize-filesystem-cloudflare
```

## Uso especifico del package


### Usa un binding Cloudflare R2

El adapter acepta el contrato estructural del bucket R2 y no importa tipos de Workers ni AWS SDK. Soporta operaciones de bytes, listados paginados acotados y streaming; los bindings R2 no pueden firmar URLs temporales ni formularios de upload directo.
...
[`@luckys_luis/nuxt-laravelize-filesystem`](../filesystem/README.es.md).
