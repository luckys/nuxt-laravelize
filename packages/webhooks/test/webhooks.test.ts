...
describe('webhooks', () => {
  ...
  it('retries 429 with Retry-After and sends without redirects', async () => {
    ...
    const processor = new OutgoingWebhookProcessor(store, { owner: 'w-1', resolveSecret, transport, resolver: async () => ['93.184.216.34'], clock })
    ...
    expect(resolveSecret).toHaveBeenCalledWith('key-1', { deliveryId: 'd-1', url: 'https://hooks.example.test/x', signal: expect.any(AbortSignal) })
  })
  it('tenant-fences signing key resolution with immutable delivery context', async () => {
    const store = new InMemoryReliabilityStore()
    await store.append(createWebhookEnvelope({ url: 'https://hooks.example.test/x', secretId: 'primary', body: null }, { id: 'tenant-delivery', occurredAt: now.toISOString(), context: { tenantId: 'tenant-1' } }))
    const resolveSecret = vi.fn(async () => secret)
    const processor = new OutgoingWebhookProcessor(store, { owner: 'worker', resolveSecret, transport: new WebhookTransportFake([{ status: 204 }]), resolver: async () => ['93.184.216.34'], clock })
    await processor.runOnce()
    expect(resolveSecret).toHaveBeenCalledWith('primary', { tenantId: 'tenant-1', deliveryId: 'tenant-delivery', url: 'https://hooks.example.test/x', signal: expect.any(AbortSignal) })
  })
