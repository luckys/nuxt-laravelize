...
```bash
pnpm add @nuxt-laravelize/notifications-database
```

```ts
// nuxt.config.ts
export default defineNuxtConfig({
  modules: ['@nuxt-laravelize/notifications-database'],
})
```
...
### Store and read database notifications

Persist only bounded, versioned JSON and let the recipient route define the tenant fence. The server helper provides cursor pagination and read-state mutations; production should replace the memory store with the Drizzle adapter.
...
[`@nuxt-laravelize/notifications-database-drizzle`](../notifications-database-drizzle/README.md), [`@nuxt-laravelize/notifications`](../notifications/README.md).
