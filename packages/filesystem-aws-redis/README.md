...
Redis and Valkey S3 upload issuance store for Nuxt Laravelize

## Install

```bash
pnpm add @luckys_luis/nuxt-laravelize-filesystem-aws-redis @luckys_luis/nuxt-laravelize-filesystem-aws ioredis
```

## Package-specific usage


### Make S3 upload confirmation restart-safe

Use the Redis issuance store when multiple Node instances can issue or confirm uploads. Its Lua transitions reserve, release, and complete one issuance with an owner token and Redis-controlled expiry.
...
[`@luckys_luis/nuxt-laravelize-filesystem-aws`](../filesystem-aws/README.md), [`@luckys_luis/nuxt-laravelize-filesystem`](../filesystem/README.md).
